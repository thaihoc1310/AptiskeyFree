const questions = [
    ...
    },
    {
        ...
        "question2_answer": "The last time I went to the sea was about six months ago. I visited a beautiful coastal town with my family. We spent the day on the beach, enjoying the warm sun and refreshing breeze. We swam in the crystal-clear water and collected shells along the shore. In the evening, we watched the sunset over the ocean, which was absolutely breathtaking. It was a relaxing and memorable experience that I will always cherish.",
        "question3": "Why do some people dislike going to the sea?",
        "question3_answer": "Not everyone enjoys going to the sea, and there are several reasons for this. Some people are afraid of deep water or cannot swim, which makes them feel unsafe near the ocean. Others dislike the feel of sand on their skin or in their clothes, as it can be quite uncomfortable. The strong sun at the beach can also cause sunburn, which is painful and unpleasant. Additionally, some people simply prefer quieter environments, like the mountains or forests, and find the noise and crowds at the beach overwhelming."
    },
    {
    ...
    },
    {
        ...
        "question3_answer": "50 years ago the news was reported by radio. People didn't have the internet and not many people had TVs. Radio was very important 50 years ago worldwide. People could listen to the radio at home, in their cars, or in the restaurant."
    },
    {
        ...
        "question2_answer": "Yes, I enjoy climbing mountains very much. There is something special about the challenge of reaching the top. The physical effort makes you feel strong and determined. When you finally reach the summit, the view is absolutely breathtaking and it makes all the hard work worthwhile. Climbing also allows you to connect with nature and enjoy the fresh mountain air. I find it very peaceful and refreshing.",
    ...
    },
    {
        ...
        "question2_answer": "The last time I shopped in a local store was just last week. I went to a nearby market to buy fresh vegetables and fruits for cooking. The market was lively, with vendors calling out their prices and displaying colorful produce. I enjoyed the experience of picking fresh items and even discovered some local specialties I hadn't tried before. It felt very personal and connected compared to shopping in a large supermarket.",
    ...
    },
    {
        ...
        "question2_answer": "Personally, I prefer watching the news on TV or online. I find it more engaging because I can see and hear the stories rather than just reading about them. Watching the news feels more immediate and I can understand the events better through video footage and live coverage. However, I also read articles online when I want more detailed information about a specific topic. Both options are useful, but watching the news fits better into my daily routine.",
    ...
    },
    {
        ...
        "question3": "On what occasions in your country do people give gifts?",
...
document.addEventListener('DOMContentLoaded', () => {
    setTotal.textContent = questions.length;
    renderSet(0, 1);

    setInput.addEventListener('change', function () {
    ...
    prevSetBtn.addEventListener('click', () => {
        if (currentSetIndex > 0) renderSet(currentSetIndex - 1, 1);
    });
    nextSetBtn.addEventListener('click', () => {
        if (currentSetIndex < questions.length - 1) renderSet(currentSetIndex + 1, 1);
    });
...
// ---- Render ----
function renderSet(setIdx, qNum) {
    stopRecording(false);
    stopReading();
    hideAnswerBox();
    resetTranscript();
    ...
    setInput.value = setIdx + 1;
    const pct = ((setIdx + 1) / questions.length * 100).toFixed(1);
    progressBar.style.width = pct + '%';

    prevSetBtn.disabled = setIdx === 0;
    nextSetBtn.disabled = setIdx === questions.length - 1;

    const set = questions[setIdx];
    pictureImg.src = set.urlpic1;
    pictureImg.style.display = '';

    updateQuestionTabs(qNum);
    questionText.textContent = set[`question${qNum}`];
    answerText.textContent   = set[`question${qNum}_answer`];
...
function switchQuestion(qNum) {
    ...
    currentQuestionNum = qNum;
    const set = questions[currentSetIndex];
    questionText.textContent = set[`question${qNum}`];
    answerText.textContent   = set[`question${qNum}_answer`];
    updateQuestionTabs(qNum);
    updateRecordBtn();
...
function updateQuestionTabs(active) {
    for (let i = 1; i <= 3; i++) {
        document.getElementById(`tab${i}`).classList.toggle('active', i === active);
    }
}
...
function updateRecordBtn() {
    if (isRecording) {
        ...
        recordBtn.innerHTML = `<i class="bi bi-stop-fill"></i> Stop (${recordTimeLeft}s)`;
    } else if (hasRecorded) {
        recordBtn.className = 'btn btn-warning';
        recordBtn.innerHTML = `<i class="bi bi-arrow-counterclockwise"></i> Ghi âm lại`;
    } else {
...
async function startRecording() {
    ...
    try {
        ...
        transcriptBox.innerHTML = '<span class="error-text">Không thể truy cập microphone. Vui lòng cấp quyền và thử lại.</span>';
        return;
    }
    ...
    if (SpeechRecognition) {
        ...
        recognition.onstart = () => {
            if (!finalTranscript) transcriptBox.innerHTML = '<span class="listening-pulse">Đang nghe...</span>';
        };
        ...
        recognition.onresult = (event) => {
            ...
            transcriptBox.innerHTML =
                (finalTranscript ? `<span class="final-text">${finalTranscript}</span>` : '') +
                (interim ? `<span class="interim-text">${interim}</span>` : '');
        };

        recognition.onerror = (e) => {
            if (e.error === 'not-allowed') {
                transcriptBox.innerHTML = '<span class="error-text">Microphone bị từ chối. Vui lòng cấp quyền trong cài đặt trình duyệt.</span>';
                return;
            }
        ...
        transcriptBox.innerHTML = '<span class="interim-text">Trình duyệt không hỗ trợ nhận diện giọng nói. Bạn vẫn có thể ghi âm và nghe lại.</span>';
    }
...
function stopRecording(showScore) {
    ...
    if (showScore) setScoreBtnVisible(true);
}
...
function resetTranscript() {
    finalTranscript = '';
    transcriptBox.innerHTML = '<span class="placeholder-text">Your speech will appear here...</span>';
}


function setScoreBtnVisible(visible) {
    scoreBtn.style.display = visible ? '' : 'none';
}
...
function toggleSampleAnswer() {
    if (answerBoxVisible) {
        ...
        answerBtn1.innerHTML = '<i class="bi bi-eye-slash me-2"></i>Ẩn đáp án';
...
function hideAnswerBox() {
    ...
    answerBtn1.innerHTML = '<i class="bi bi-chat-text me-2"></i>Sample Answer';
    stopReading();
}
...
function startReading() {
    ...
    utter.onend = () => {
        ...
        readBtn.innerHTML = '<i class="bi bi-volume-up-fill"></i> Đọc';
    };
    window.speechSynthesis.speak(utter);
    currentlyReading = true;
    readBtn.classList.add('reading');
    readBtn.innerHTML = '<i class="bi bi-stop-fill"></i> Dừng';
}
...
async function scoreAnswer() {
    const userAnswer = transcriptBox.textContent.trim();
    const set = questions[currentSetIndex];
    const qText = set[`question${currentQuestionNum}`];

    if (!userAnswer || userAnswer === 'Your speech will appear here...') {
        alert('Chưa có câu trả lời để chấm điểm.');
        return;
    }
