...
},
{
    ...
    question2_answer: "Yes, I really enjoy spending time with animals, particularly with dogs and cats. I believe they share emotions similar to humans, and they play a significant role in helping me unwind after a stressful day. When I return home after a hectic workday, I like to engage in playful moments with my pets. They react with excitement, wagging their tails and expressing joy whenever they hear my voice from a distance. Animals, especially pets, have this incredible ability to create a positive and stable atmosphere in my life. They bring joy and contribute to a sense of family closeness. Overall, I find animals to be not just wonderful companions but also a source of comfort and happiness in my everyday life.",
...
},
{
    ...
    question1: "Describe the picture?",
    question1_answer: "As I can see in the first picture, there is a father and his son playing with a horse. The father is wearing a shirt, and the son is wearing a shirt. The boy looks very happy, maybe because it's the first time he has seen a horse. Looking at the background, I think they are in a park and enjoying their day off.\n\nOn the other hand, in the second picture, I see a teacher with her students. The teacher is wearing a shirt, and she is guiding the kids to play with a snake on the table. The kids look excited and happy.\n\nBoth pictures show children interacting with animals, but in different ways. I prefer the first picture because I enjoy seeing a happy family having fun with animals. However, the second picture also looks nice because it shows kids learning about animals in a fun way.",
    question2: "Do you like interacting with animals?",
...
// ── DOM refs ───────────────────────────────────────────────────────────────
const pictureImg1   = document.getElementById('pictureImg1');
const pictureImg2   = document.getElementById('pictureImg2');
...
// ── Render ─────────────────────────────────────────────────────────────────
function renderSet(idx, qNum) {
    currentSetIndex = idx;
    currentQuestionNum = qNum;

    const set = questions[idx];
    pictureImg1.src = set.urlpic1;
    pictureImg2.src = set.urlpic2;
    pictureImg1.style.display = '';
    pictureImg2.style.display = '';

    setInput.value = idx + 1;
    setTotal.textContent = questions.length;
    progressBar.style.width = ((idx + 1) / questions.length * 100) + '%';

    prevSetBtn.disabled = idx === 0;
    nextSetBtn.disabled = idx === questions.length - 1;

    switchQuestion(qNum, true);
    resetRecording();
    hideAnswerBox();
}

function switchQuestion(qNum, forceReset) {
    currentQuestionNum = qNum;
    ...
    let qText = set[qKey] || '';
    // Strip leading "2. " or "3. " prefix if present
...
function toggleSampleAnswer() {
    if (answerBoxVisible) {
        ...
        const set = questions[currentSetIndex];
        const aKey = 'question' + currentQuestionNum + '_answer';
        answerText.innerHTML = set[aKey] || '';
...
function toggleRead() {
    if (isSpeaking) {
        ...
        const text = answerText.innerText || answerText.textContent;
        if (!text.trim()) return;
...
async function startRecording() {
    finalTranscript = '';
    transcriptBox.innerHTML = '<span class="listening-pulse"><i class="bi bi-mic-fill me-1"></i>Đang nghe...</span>';
    scoreBtn.style.display = 'none';
    audioPlayerSection.style.display = 'none';
    ...
    try {
        ...
        const msg = err.name === 'NotAllowedError' || err.name === 'PermissionDeniedError'
            ? 'Bạn chưa cấp quyền micro. Vui lòng cho phép trong cài đặt trình duyệt.'
            : 'Không thể truy cập micro: ' + err.message;
        transcriptBox.innerHTML = `<span class="error-text">${msg}</span>`;
        return;
    }

    // MediaRecorder with cross-platform MIME detection
    ...
    if (SR) {
        ...
        recognition.onerror = function(e) {
            if (e.error !== 'no-speech' && e.error !== 'aborted') {
                transcriptBox.innerHTML += `<br><span class="error-text">Lỗi nhận dạng: ${e.error}</span>`;
            }
        };
...
function stopRecording() {
    ...
    if (finalTranscript.trim()) {
        transcriptBox.innerHTML = `<span class="final-text">${finalTranscript.trim()}</span>`;
    } else {
        transcriptBox.innerHTML = '<span class="placeholder-text">Không nhận được giọng nói. Thử lại.</span>';
    }
...
async function scoreAnswer() {
    ...
    try {
        ...
        modalBody.textContent = 'Lỗi kết nối: ' + err.message;
    }
}
